...
custom_imports = dict(
    imports=['projects.BEVFusion.bevfusion.my_collate',
             ...
             'projects.BEVFusion.bevfusion.custom_data_preprocessor',
             'projects.BEVFusion.bevfusion.project_3dbox_to_image',
             ],
...
model = dict(
    ...
    # # --- 2. 아래의 공식 FCOSHead 설정을 붙여넣으세요. ---
    # img_bbox_head=dict(
    #     type='mmdet.FCOSHead', # 👈 공식 모델은 FCOSHead를 사용합니다
    #     num_classes=10,
    #     in_channels=256,
    #     stacked_convs=4,
    #     feat_channels=256,
    #     strides=[8, 16, 32, 64], # 👈 FPN 출력에 맞는 strides
...
#         with_bbox_3d=True,
#         with_label_3d=True,
#         with_bbox=False,       # ◀-- 원본 2D GT는 로드하지 않음
#         with_label=False,      # ◀-- 원본 2D 라벨은 로드하지 않음
#         with_attr_label=False),
#     dict(
#         type='CustomImageAug3D', # ◀-- 이미지 증강 먼저 수행
#         final_dim=[256, 704],
#         resize_lim=[0.38, 0.55],
#         bot_pct_lim=[0.0, 0.0],
#         rot_lim=[-5.4, 5.4],
...
#         is_train=True),
#     dict(
#         type='BEVFusionGlobalRotScaleTrans', # ◀-- 3D 공간 증강
#         scale_ratio_range=[0.9, 1.1],
#         rot_range=[-0.78539816, 0.78539816],
#         translation_std=0.5),
#     dict(type='BEVFusionRandomFlip3D'),
...
#     # ✨ [핵심] 모든 증강이 끝난 후, 3D 박스를 2D로 투영하여 GT 생성
#     dict(type='BEVDetProject3DBoxToImage'),
...
#             'gt_bboxes', 'gt_labels', # ◀-- 투영으로 생성된 2D GT 포함
#         ],
#         meta_keys=[
#             'img_shape', 'ori_shape', 'pad_shape', 'scale_factor',
#             'cam2img', 'ori_cam2img', 'lidar2cam', 'lidar2img', 'cam2lidar',
...
optim_wrapper = dict(
    ...
    paramwise_cfg=dict(
        custom_keys={
            ...
            'img_backbone': dict(lr_mult=0.01, decay_mult=1.0),
...
default_hooks = dict(
    ...
    checkpoint=dict(
        type='CheckpointHook',
        interval=1000,          # 💾 1000 이터레이션마다 저장
        by_epoch=False,         # 👈 이터레이션 기준
        max_keep_ckpts=3,       # 📂 최신 3개만 유지하고 나머지는 자동 삭제
        save_best='auto',       # 🏆 성능이 가장 좋은 모델을 'best_...pth'로 별도 저장
        rule='greater'          # mAP, Accuracy 등 높을수록 좋은 지표일 경우 'greater'
    ))
del _base_.custom_hooks

# load_from =  "data/weights/bevfusion_lidar-cam_voxel0075_second_secfpn_8xb4-cyclic-20e_nus-3d-5239b1af.pth"
# load_from =  "work_dirs/bevfusion_lidar-cam_voxel0075_second_secfpn_8xb4-cyclic-20e_nus-3d_moon/iter_5000.pth"
load_from = None
resume_from = None
...
vis_backends = [
    ...
    dict(type='TensorboardVisBackend') # TensorBoard 백엔드 추가
]

# visualizer에 위에서 정의한 백엔드 리스트를 연결
visualizer = dict(
