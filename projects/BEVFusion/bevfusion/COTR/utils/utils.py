...
def float_image_resize(img, shape, interp='bilinear'):
    ...
    for l in img:
        ...
        #l = misc.imresize(l, shape, interp=interp, mode='F')
        l = cv2.resize(l, shape, interpolation=cv2.INTER_LINEAR)
        layers.append(l)
    if missing_channel:
...
def visualize_corrs(img1, img2, corrs, mask=None):
    ...
    #img1 = misc.imresize(img1, scale1)
    #img2 = misc.imresize(img2, scale2)
